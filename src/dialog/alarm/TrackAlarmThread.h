...
private:
    void processAlarms();
    void updataAlarmTrackToDB(QSet<int> trackID, AlarmRule info,int type,int radarSourceId = 0);
    ...
    void SaveToDB(AlarmRule info, int id, qint64 uniqueid, float lat, float lon, float speed, float dir, float dis, int TargetType, int threatScore, int timestampSec, int radarSourceId = 0);
    ...
    // �ٶ��ж������Ը���
    QMap<int, bool> m_mapSpeedLastCheck;
